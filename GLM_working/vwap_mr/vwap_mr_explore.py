...
=================================
VWAP 均值回归·上午场 (10:00-11:00 ET) —— 描述统计 + 粗版策略期望。只读数据, 不改任何主线代码。
...
口径
  ...
  - 描述统计: 窗口内首次 |dev|≥k 后, 价格首次触碰 VWAP 的概率 (11点前/12点前/收盘前) 与发生时间。
...
输出: 终端报告 + results/vwap_mr_reversion.csv / vwap_mr_grid.csv / vwap_mr_byyear.csv
"""
...
DATA_PATH = ORB_DIR / "nq_continuous_1m.parquet"
...
COST_PT = 1.0                  # MNQ 往返成本 (点): 每边 ($0.5 佣金 + 1tick 滑 $0.5) / $2每点
...
def build_atr_map() -> dict:
    ...
    df = pd.read_parquet(ORB_DIR / "nq_5min_rth.parquet").tz_convert(ET)
    day = (
        df.resample("1D")
...
def first_breach(dev: np.ndarray, ts_t: list, k: float) -> int | None:
    """窗口 [10:00,11:00) 内首个 |dev|≥k 的 bar 下标。
    注意 session 含前一晚 18:00 起的 bar, 不能按 t≥11:00 提前 break (会在 18:00 首根就跳出)。"""
...
def walk_trade(day: dict, i0: int, sign: int, s: float) -> tuple[float, str, int]:
...
def run():
    ...
    for sess, grp in df.groupby("sess"):
        ...
        if i0 is not None:
            ...
            if j is None:
                ...
                rev_rows.append((bucket, dev_r[i0] if np.isfinite(dev_r[i0]) else np.nan,
                                 1 if tj < T_WIN_END else 0,
                                 1 if tj < T_NOON else 0,
                                 1.0))
        ...
        for k in K_GRID:
            ...
            for s in S_GRID:
                net, reason, hold = walk_trade(day, ik, sign, s)
                if np.isfinite(net):
    ...
    print("=" * 78)
    print(f"VWAP 均值回归·上午场 (10:00-11:00 ET)  样本 {df.index[0].date()} ~ {df.index[-1].date()}, {n_days} 个交易日")
    print("主锚: ETH 18:00 ET (平台标准 VWAP);  敏感性: RTH 9:30 锚")
    print("=" * 78)

    print(f"\n--- 10:00 时刻偏离 |close-VWAP| ---")
    ...
    q = P(sig10_pt, [50])[0]
    print(f"σ(10:00) 中位 = {q:.1f} pt  (VWAP band 的 1σ 宽度)")
    ...
    # 回归概率表
    rows = pd.DataFrame(rev_rows, columns=["bucket", "dev_rth_anchor", "by11", "by12", "by_close"])
    print(f"\n--- 首破≥1σ 后的 VWAP 回归概率 (主锚, 按首次突破档位) ---")
    print(f"{'档位(σ)':<12}{'n':>5}{'P(11点前)':>10}{'P(12点前)':>10}{'P(收盘前)':>10}")
    out_rows = []
    for b in K_BUCKETS:
        ...
        print(f"{label:<12}{len(sub):>5}{sub['by11'].mean():>10.1%}{sub['by12'].mean():>10.1%}"
              f"{sub['by_close'].mean():>10.1%}")
        out_rows.append(dict(bucket=label, n=len(sub), p_by_1100=sub["by11"].mean(),
                             p_by_1200=sub["by12"].mean(), p_by_close=sub["by_close"].mean()))
    if rth_sens:
        print(f"(敏感性: RTH 9:30 锚, 首破≥1.5σ 后收盘前回归概率 = {np.mean(rth_sens):.1%}, n={len(rth_sens)})")
    pd.DataFrame(out_rows).to_csv(RESULTS / "vwap_mr_reversion.csv", index=False)
    ...
    for name, cond in [("首破≥1.0σ", lambda dv: dv >= 1.0), ("首破≥2.0σ", lambda dv: dv >= 2.0)]:
        ...
        b = pd.Series([f"{t.strftime('%H')}-{int(t.strftime('%H'))+1}" if t < dtime(14, 0)
                       else ("14-15:55" if t < T_EOD else "?") for t in times]).value_counts()
        seg = {s: b.get(s, 0) for s in ["10-11", "11-12", "12-13", "13-14", "14-15:55"]}
        never = n_all - len(times)
        print(f"{name}: n={n_all}  " + "  ".join(f"{k}:{v/n_all:.0%}" for k, v in seg.items())
              + f"  未回归:{never/n_all:.0%}")

    # 粗版策略网格
    print(f"\n--- 粗版策略: 窗口首破 kσ 反向入场, 目标 VWAP, 止损 s·σ, 15:55 收盘平 (净R已扣 MNQ 往返 1.0pt) ---")
    print(f"{'k':>4}{'s':>4}{'n':>5}{'胜率':>7}{'均R净':>8}{'中位R':>8}{'p05':>7}{'最差R':>8}{'止损率':>7}{'EOD率':>7}{'中位持仓m':>9}")
    g_rows = []
    for k in K_GRID:
        for s in S_GRID:
            tr = trades_grid[(k, s)]
            ...
            print(f"{k:>4.1f}{s:>4.1f}{len(r):>5}{(r>0).mean():>7.1%}{r.mean():>8.2f}"
                  f"{np.median(r):>8.2f}{np.percentile(r,5):>7.2f}{r.min():>8.1f}"
                  f"{reasons.count('stop')/len(r):>7.1%}{reasons.count('eod')/len(r):>7.1%}"
                  f"{np.median(holds):>9.0f}")
            g_rows.append(dict(k=k, s=s, n=len(r), win=(r > 0).mean(), avg_net_r=r.mean(),
                               med_r=np.median(r), p05=np.percentile(r, 5), worst=r.min(),
                               stop_rate=reasons.count("stop") / len(r),
                               eod_rate=reasons.count("eod") / len(r)))
    pd.DataFrame(g_rows).to_csv(RESULTS / "vwap_mr_grid.csv", index=False)
    ...
    hy.to_csv(RESULTS / "vwap_mr_byyear.csv", index=False)
    print(f"\nCSV → {RESULTS}/vwap_mr_reversion.csv, vwap_mr_grid.csv, vwap_mr_byyear.csv")
