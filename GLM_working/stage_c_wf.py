...
def run_window(job):
    ...
    train = {}
    for s, b in grid:
        m = core.run_backtest(s, b, risk, tr, CAPITAL)
        train[(s, b)] = m
    ...
    for s, b in grid:   # 整个网格都跑测试段 → 固定组合 OOS 路径
        m = core.run_backtest(s, b, risk, te, CAPITAL)
        test[(s, b)] = m
...
def main():
    ...
    print(f"Stage C Walk-Forward: 网格 {len(stops)} 止损 × {len(bes)} 保本 = {len(grid)} 组合, "
          f"risk={args.risk:.1%}, 本金 ${CAPITAL:,}, 训练段按 Calmar 选 θ* "
          f"(n_trades>={MIN_TRADES}), {WORKERS} 进程 × 6 窗口", flush=True)

    jobs = [dict(w, stops=stops, bes=bes, risk=args.risk) for w in WINDOWS]
    t0 = time.time()
    ...
    out_csv = "results/stage_c_wf_oos.csv"
