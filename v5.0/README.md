...
**隔离纪律（2026-09-17 实装）**：`test_atr_source`（用例 5/6）与 `verify_live` 都在引擎内跑**真实** live 适配层，故已把滑点落盘（`SLIP_CSV`）与运行日志（`LOG_DIR`）重定向到临时目录，并断言 `v5.0/live_slippage.csv`、`v5.0/logs/`、`archive/live/logs/` **零写入** —— 这几条路径是实盘审计底稿（orb-monitor 只读消费，见其 README 数据契约），跑测试绝不许往里写。新加引擎内用例请照抄这个模式。
