...
orb_backtest_v5.py
==================
NQ 期货 5 分钟 ORB 日内策略 —— NautilusTrader 版本 (v5)

v5: 在 v4 基础上, 仓位改为风险定仓 + 单笔最大手数上限
  仓位: 手数 = min( floor(equity × 1% / (止损点数 × $20)),  40 手 )
     - 第一项 = 风险定仓 (止损打到亏 1%)
     - 第二项 = 单笔最大 40 手上限

其余与 v4 一致:
  - 时段: 2016-01-01 ~ 2023-02-17 (论文窗口)
  - 止损: 5% × 14日ATR (前一日, 无未来函数)
  - 止盈: 无 (持有至收盘)
  - 方向/入场: K1 阴阳定方向, K1 收盘价市价入场
  - 收盘平仓, 手续费 $0.5/手/边, 无滑点, 每日最多 1 笔

用法: cd ORB_strategy && ../.venv/bin/python orb_backtes_v5.py
"""
...
if __name__ == "__main__":
    ...
    venue = Venue(VENUE)
    engine = BacktestEngine(config=BacktestEngineConfig(trader_id=TraderId("ORB-BT-005")))
    engine.add_venue(
        venue=venue,
    ...
    create_tearsheet(engine, output_path="html_output/orb_report_v5.html", title="NQ 5min ORB v5 回测报告 (2010-2026)")
    print("\n可视化报告已生成: html_output/orb_report_v5.html")
