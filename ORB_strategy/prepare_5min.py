...
「NQ 常规时段 9:30-16:00 ET 的 5 分钟 K 线」, 供 NautilusTrader 回测使用。

步骤:
  1) UTC → America/New_York (ET, 自动处理夏令时)
  2) 过滤 RTH: 09:30 <= 开盘时间 < 16:00
  3) 1 分钟 → 5 分钟重采样(左闭合, 对齐 9:30), 聚合 OHLCV
  4) 输出 nq_5min_rth.parquet (索引转回 UTC, 供引擎使用)

注意: OHLCV 是"稀疏"数据, 重采样后若某 5 分钟窗口无成交会被丢弃(NQ RTH 极罕见)。
用法: cd ORB_strategy && ../.venv/bin/python prepare_5min.py
"""
...
print("[3/3] 重采样为 5 分钟 (左闭合, 对齐 9:30) ...", flush=True)
res = (
...
res.to_parquet("nq_5min_rth.parquet")

# 摘要
days = res.index.normalize().nunique()
print(f"\n===== 摘要 =====")
print(f"5 分钟 bar 数 : {len(res):,}")
print(f"交易日数      : {days}")
...
print(f"\n示例日 {sample_day.date()} 前 4 根 5 分钟 K 线 (ET):")
