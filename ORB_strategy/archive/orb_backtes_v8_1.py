# -*- coding: utf-8 -*-
"""
orb_backtest_v8_1.py
====================
NQ 期货 5 分钟 ORB 日内策略 —— NautilusTrader 版本 (v8.1)

v8.1: 在 v8 基础上, 入场窗口提前到 9:30 (含第一根 5 分钟 K 线)
...
本脚本末尾直接生成两张图表:
  1) html_output/orb_report_v8_1.html  统计报告 (tearsheet)
  2) html_output/orb_chart_v8_1.html   K 线图 (Lightweight Charts)

用法: cd ORB_strategy && ../.venv/bin/python orb_backtes_v8_1.py
"""
import json
...
if __name__ == "__main__":
    ...
    venue = Venue(VENUE)
    engine = BacktestEngine(config=BacktestEngineConfig(trader_id=TraderId("ORB-BT-081")))
    engine.add_venue(
        venue=venue,
    ...
    # ---- 生成两张图表 ----
    report_path = "html_output/orb_report_v8_1.html"
    create_tearsheet(engine, output_path=report_path, title="NQ 5min ORB v8.1 回测报告 (2016-2023)")

    trades = extract_trades(engine)
    bar_list = extract_bars(bars)
    chart_path = "html_output/orb_chart_v8_1.html"
