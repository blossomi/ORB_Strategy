...
orb_backtest_v6.py
==================
NQ 期货 5 分钟 ORB 日内策略 —— NautilusTrader 版本 (v6)

v6 在 v5 基础上新增三项改动:
  1) 10R 止盈: 入场后挂 10R 限价止盈单, 收盘前到达 10R 立即止盈
  2) 1R 保本: 入场后若浮盈达到 1R, 把止损移到「保本 + 缓冲」(缓冲覆盖点差+手续费)
  3) 回测年限: 放宽到全部数据 2010-06-07 ~ 2026-08-28 (不再局限于论文 2016-2023 窗口)

其余与 v5 一致:
  - 止损: 5% × 14日ATR (前一日, 无未来函数)
  - 方向/入场: K1 阴阳定方向, K1 收盘价市价入场
  - 仓位: 风险定仓(1%复利) + 单笔最大 40 手上限
  - 收盘平仓, 手续费 $0.5/手/边, 无滑点, 每日最多 1 笔

假设说明 (5分钟K线粒度):
  - 止盈/止损由 NautilusTrader 撮合引擎按 K 线 open→high→low→close 顺序撮合(默认 high 优先)。
  - 「1R 保本」在 K 线收盘时检测(high 触及 1R 才移动), 移动后从下一根 K 线生效;
    若同一根 K 线先触 1R 又回落到初始止损, 按初始止损平仓(偏保守)。
  - 出场单均 reduce_only, 避免同一根 K 线同时触发止盈+止损时反向开仓。
  - 收盘平仓用「当日实际最后一根 K 线」(常规 15:55, 半日/提前收盘日更早), 避免持仓跨夜。

用法: cd ORB_strategy && ../.venv/bin/python orb_backtes_v6.py
"""
...
BE_R_MULTIPLE = 1.0                              # 保本触发 = 浮盈 1R
BE_BUFFER_TICKS = 2                              # 保本缓冲 = 2 tick = 0.50pt (点差 0.25 + 手续费 ~0.05)

START_DATE = "2010-06-07"                        # 全数据起始 (RTH 首根 K 线)
END_DATE = "2026-08-28"                          # 全数据截止 (RTH 末根 K 线)
...
class OrbStrategyConfig(StrategyConfig):
    ...
    be_r_multiple: float = 1.0                    # 保本触发 = 1R
...
class OrbStrategy(Strategy):
    def __init__(self, config: OrbStrategyConfig, atr_map: dict[ddate, float],
        ...
        self._trade = None                    # 当前持仓状态 dict (side/qty/r/entry_px/stop_order/tp_order/stop_moved)
    ...
    def _check_breakeven(self, bar: Bar):
        trade = self._trade
...
if __name__ == "__main__":
    ...
    venue = Venue(VENUE)
    engine = BacktestEngine(config=BacktestEngineConfig(trader_id=TraderId("ORB-BT-006")))
    engine.add_venue(
        venue=venue,
    ...
    print(f"===== 10R 止盈: {strategy.n_tp_hits:,}  |  移到保本: {strategy.n_be_moves:,} =====")
    print(f"===== 初始止损出场: {strategy.n_stopped:,}  |  保本止损出场: {strategy.n_be_exits:,}  |  收盘平仓: {strategy.n_eod:,} =====")
    ...
    create_tearsheet(engine, output_path="html_output/orb_report_v6.html", title="NQ 5min ORB v6 回测报告 (2010-2026 全数据, 10R止盈 + 1R保本)")
    print("\n可视化报告已生成: html_output/orb_report_v6.html")
