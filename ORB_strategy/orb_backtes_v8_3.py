# -*- coding: utf-8 -*-
"""
orb_backtest_v8_3.py
====================
NQ 期货 5 分钟 ORB 日内策略 —— NautilusTrader 版本 (v8.3)

v8.3: 入场改为「收盘价判断突破」, 修复 v8.2 的方向歧义
  ...
  - 止盈: 无 (持有至收盘平仓)
  - 仓位: 以损定仓 floor(equity × 1% / (止损点数 × $20)), 单笔最大 1700 手

本脚本末尾直接生成两张图表:
  1) html_output/orb_report_v8_3.html  统计报告 (tearsheet)
  2) html_output/orb_chart_v8_3.html   K 线图 (Lightweight Charts)

用法: cd ORB_strategy && ../.venv/bin/python orb_backtes_v8_3.py
"""
import json
...
T_WIN_START = dtime(9, 30)                         # 入场窗口开始 (第一根 5 分钟 K 线)
T_WIN_END = dtime(9, 50)                           # 入场窗口结束 (9:50 前无方向则不交易)
...
版本       : v8.3
标的       : NQ 期货 (5分钟 RTH)
时段       : 2016-01 ~ 2026-08-30
入场       : 收盘价判断突破 (窗口 9:30-9:50, 逐根 K 线)
             收盘价涨破 9:00-9:30 区间高点 → 做多
             收盘价跌破 9:00-9:30 区间低点 → 做空
             第一根在区间内则等下一根, 9:50 前无方向则不交易
修复       : 用收盘价(单一值)判断方向, 消除"最高/最低价同时越界"的歧义
             (v8.1/v8.2 用 bar.high/bar.low 触及判断, 26% 交易日两边同时突破被误判为做多)
止损       : 5% × 14日ATR (前一日, 无未来函数)
止盈       : 无 (持有至收盘平仓)
仓位       : floor(equity × 1% / (止损点数 × $20)), 单笔最大 1700 手
手续费     : $0.5/手/边, 无滑点"""
...
if __name__ == "__main__":
    ...
    venue = Venue(VENUE)
    engine = BacktestEngine(config=BacktestEngineConfig(trader_id=TraderId("ORB-BT-083")))
    engine.add_venue(
        venue=venue,
    ...
    # ---- 生成两张图表 ----
    report_path = "html_output/orb_report_v8_3.html"
    create_tearsheet(engine, output_path=report_path, title="NQ 5min ORB v8.3 回测报告 (2016-2026)")

    trades = extract_trades(engine)
    bar_list = extract_bars(bars)
    chart_path = "html_output/orb_chart_v8_3.html"
