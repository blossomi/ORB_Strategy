...
orb_backtest_v7.py
==================
NQ 期货 15 分钟 ORB 日内策略 —— NautilusTrader 版本 (v7)

v7: 在 v6.02 基础上【把 5 分钟 K 线改成 15 分钟 K 线】:
  - 方向/入场: 开盘后第一根 15 分钟 K 线 (9:30-9:45) 阴阳定方向, 其收盘价(9:45)市价入场
    (原 v6.02 用第一根 5 分钟 K 线 9:30-9:35 定方向)

其余与 v6.02 一致:
  - 止损: 5% × 14日ATR (前一日, 无未来函数), 全程固定不移动
  - 止盈: 10R 限价止盈单, 收盘前到达 10R 立即止盈
  - 收盘: 当日最后一根 15 分钟 K 线收盘平仓
...
假设说明 (15分钟K线粒度):
  ...
  - 收盘平仓用「当日实际最后一根 K 线」(常规 15:45, 半日/提前收盘日更早), 避免持仓跨夜。
  - 当日仅 1 根 K 线的数据异常日(如 2013-09-20)直接跳过, 避免入场后无法收盘而跨夜。

用法: cd ORB_strategy && ../.venv/bin/python orb_backtes_v7.py
"""
...
# 配置
# ---------------------------------------------------------------------------
DATA_PATH = "nq_15min_rth.parquet"
...
T_K1 = dtime(9, 30)                              # 当日第一根 15 分钟 K 线 (9:30-9:45)
...
def build_day_last_bar_map() -> dict[ddate, dtime]:
    """返回 {ET 交易日 date: 当日最后一根 15 分钟 K 线的 ET 时间}。

    常规 RTH 最后根为 15:45; 但半日/提前收盘日最后根更早(如 11:15/12:45),
    若仍按固定时间收盘, 这些日的持仓会跨夜。用实际最后一根 K 线收盘可避免跨夜。
    """
    df = pd.read_parquet(DATA_PATH).tz_convert(ET)
...
class OrbStrategy(Strategy):
    def __init__(self, config: OrbStrategyConfig, atr_map: dict[ddate, float],
        ...
        self.n_degenerate = 0                 # 仅 1 根 K 线的数据异常日跳过天数
    ...
    def on_bar(self, bar: Bar):
        ...
        if t == T_K1:
            ...
            # 当日仅 1 根 K 线(数据异常): 入场后无法收盘会跨夜, 跳过
...
# ---------------------------------------------------------------------------
# 数据: 读 15 分钟 parquet (过滤到窗口) → Nautilus Bar + 合成连续合约
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    print("[2/4] 加载 15 分钟数据...", flush=True)
    ...
    venue = Venue(VENUE)
    engine = BacktestEngine(config=BacktestEngineConfig(trader_id=TraderId("ORB-BT-007")))
    engine.add_venue(
        venue=venue,
    ...
    print(f"===== 仅1根K线数据异常日跳过: {strategy.n_degenerate:,} 天 =====")
    print("===== 账户报告(首/尾) =====")
    ...
    create_tearsheet(engine, output_path="html_output/orb_report_v7.html", title="NQ 15min ORB v7 回测报告 (2016-2026, 10R止盈 + 无保本)")
    print("\n可视化报告已生成: html_output/orb_report_v7.html")
